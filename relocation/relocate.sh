#!/bin/bash
# Change CSV file and update DHCP reservation conf file
#
...
### DHCP reconfigure part ###

#if [ -e $dhcp_conf ]
#then
#	# replace the old_mac with the new_mac inside DHCP conf file
#	swap_macs $old_mac $new_mac $dhcp_conf
#	printf "See the new state below:\n"
#	grep -A2 -B1 "${old_mac}" $dhcp_conf
